...
class IntermediateFusionDatasetIrregularDETR3D(basedataset_backup.BaseDataset):
    """
    ...
    def __init__(self, params, visualize, train=True):
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # regular_timestamps2 = self.extract_timestamps(yaml_files)

            # assert (regular_timestamps == regular_timestamps2)
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            assert ('lidar_pose' in data[cav_id]['curr']['params'])
            # # 没有 lidar pose
            ...
            # data[cav_id]['debug'] = {}
            # data[cav_id]['debug']['scene'] = scenario_index
    ...
    def __getitem__(self, idx):
        ...
        # hwl ----> lwh
        object_stack[:,[3,5]] = object_stack[:,[5,3]]
        gt_bboxes_3d = LiDARInstance3DBoxes(
            object_stack,
            box_dim=object_stack.shape[-1],
            origin=(0.5, 0.5, 0.5))
        
        gt_labels_3d = np.zeros(object_stack.shape[0]).astype(int)
        ...
        # from IPython import embed
        # embed(header='processed_data_dict')
        try:
            processed_data_dict['ego'].update({'avg_sample_interval':\
                sum(past_k_sample_interval_stack) / len(past_k_sample_interval_stack)})
            processed_data_dict['ego'].update({'avg_time_delay':\
                sum(past_k_time_diffs_stack) / len(past_k_time_diffs_stack)})
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, idx):
        ...
        # if curr_feature['voxel_coords'].shape[0] ==0:
        #     from IPython import embed
        #     embed(header='0')
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            gt_bboxes_3d.append(ego_dict['gt_bboxes_3d'])
            gt_labels_3d.append(torch.from_numpy(ego_dict['gt_labels_3d']))
    ...
    def collate_batch_test(self, batch):
        # from IPython import embed
        # embed(header='collate_batch_test')
