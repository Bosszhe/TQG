...
from opencood.data_utils.datasets import basedataset_scope
...
class IntermediateFusionDatasetSCOPE(basedataset_scope.BaseDataset):
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetSCOPE, self). \
    ...
    def cal_distance(self,select_dict,idx):
        distance_dict = {}
        void_lidar_dict = {}
        cur_data_dict = select_dict[idx]
        for cav_id in cur_data_dict.keys():
            distance_dict[cav_id] = []
            void_lidar_dict[cav_id] = []
        
        for cav_id, cav_content in cur_data_dict.items():
            if cav_content['ego']:
                ego_id = cav_id
                ego_lidar_pose = cav_content['params']['lidar_pose']
                break
        assert cav_id == list(cur_data_dict.keys())[0], "The first element in the OrderedDict must be ego"
        assert ego_id != -1
        assert len(ego_lidar_pose) > 0

        
        for index,time_data_dict in select_dict.items():
            for cav_id, cav_content in time_data_dict.items():
                
                dd = \
                math.sqrt((cav_content['params']['lidar_pose'][0] -
                        ego_lidar_pose[0]) ** 2 + (
                                cav_content['params'][
                                    'lidar_pose'][1] - ego_lidar_pose[
                                    1]) ** 2)
                distance_dict[cav_id].append(dd)

                # cav_content, void_lidar = self.get_item_single_car(
                #     cav_content,
                #     ego_lidar_pose)
                
                # void_lidar_dict[cav_id].append(void_lidar)

        return distance_dict,void_lidar_dict
    ...
    def __getitem__(self, idx):
        ...
        distance_dict, void_lidar_dict=self.cal_distance(select_dict,idx)
        ...
        for index,base_data_dict in select_dict.items():
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                if all(dd > opencood.data_utils.datasets.COM_RANGE for dd in distance_dict[cav_id]):
                    # print(distance)
                    continue
                ...
                ego_flag.append(selected_cav_base['ego'])
        ...
        if processed_data_list[0]['ego']['cav_num'] != processed_data_list[1]['ego']['cav_num']:
            from IPython import embed
            embed(header='budengyu')
    ...
    def collate_batch_train(self, batch):
        ...
        # from IPython import embed
        # embed(header='collate_batch_train')
