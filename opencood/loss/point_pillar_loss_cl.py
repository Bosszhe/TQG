...
class PointPillarLossCL(nn.Module):
    def __init__(self, args):
        super(PointPillarLossCL, self).__init__()
    ...
    def forward(self, output_dict, target_dict, prefix=''):
        ...
        total_loss = reg_loss + conf_loss + simi_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f || Simi Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), conf_loss.item(), reg_loss.item(), simi_loss.item()))
        else:
