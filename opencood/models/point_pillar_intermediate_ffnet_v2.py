...
class PointPillarIntermediateFFNetV2(nn.Module):
    def __init__(self, args):
        super(PointPillarIntermediateFFNetV2, self).__init__()
        print('PointPillarIntermediateFFNetV2.__init__')
    ...
    def forward(self, data_dict_list):
        ...
        # from IPython import embed
        # embed()
        past_bev_trans = torch.cat([past_bev, cur_bev], dim=1)

        past_bev_trans = self.encoder(past_bev_trans)
        batch_dict['spatial_features'] = past_bev_trans + past_bev
