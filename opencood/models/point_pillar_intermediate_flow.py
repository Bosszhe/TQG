...
class PointPillarIntermediateFlow(nn.Module):
    def __init__(self, args):
        super(PointPillarIntermediateFlow, self).__init__()
        print('PointPillarIntermediateFlow.__init__')

        # PIllar VFE
        self.past_pillar_vfe = PillarVFE(args['pillar_vfe'],
                                    num_point_features=4,
                                    voxel_size=args['voxel_size'],
                                    point_cloud_range=args['lidar_range'])
        self.past_scatter = PointPillarScatter(args['point_pillar_scatter'])
    ...
    def forward(self, data_dict_list):
        ...
        batch_dict = self.past_pillar_vfe(batch_dict)
        batch_dict = self.past_scatter(batch_dict)
