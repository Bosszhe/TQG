...
class PointPillarIntermediateScope(nn.Module):
    def __init__(self, args):
        super(PointPillarIntermediateScope, self).__init__()
    ...
    def forward(self, data_dict_list):
        ...
        if self.async_mode:
            ...
            batch_dict = {'voxel_features': cur_voxel_features,
        ...
        if not batch_dict['record_len'].sum() == batch_dict['spatial_features'].shape[0]:
        # if not cur_record_len.sum() == past_record_len.sum():
            from IPython import embed
            embed(header='ERROR')
