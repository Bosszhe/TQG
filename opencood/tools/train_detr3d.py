...
def main():
    ...
    # opencood_validate_dataset = build_dataset(hypes, visualize=True, train=True)
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            # as well
            if not opt.half:
                final_loss, final_loss_dict = model(batch_data['ego'])
            ...
            # embed()
            criterion.logging(epoch, i, len(train_loader), writer, final_loss_dict, pbar=pbar2)
            pbar2.update(1)
