...
def main():
    ...
    if opt.eval_epoch == "loop": 
        epoch_id_list = list(range(17,35))
    ...
    for epoch_id in epoch_id_list:
        ...
        with open(os.path.join(saved_path, 'result.txt'), 'a+') as f:
            ...
            if opt.comm_thre is not None:
                msg = 'Split : {} | Epoch: {} | AP @0.3: {:.04f} | AP @0.5: {:.04f} | AP @0.7: {:.04f} | comm_rate: {:.06f} | comm_thre: {:.04f} | score_threshold: {:.02f} | xyz_std: {:.01f} | ryp_std: {:.01f}\n'.format(split, epoch_id, ap_30, ap_50, ap_70, comm_rates, opt.comm_thre,score_threshold,opt.xyz_std,opt.ryp_std)
            f.write(msg)
            print(msg)
